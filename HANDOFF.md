...
> （訓練＋散布授權已於 2026-08-04 確認合法，標明出處即可）。新切分指令：
>
> ```bash
> python3 scripts/split_data.py --use-all
> ```
>
> - 文化部語料庫 **5,272 句全數納入訓練**（不再留存 test_corpus、不濾單詞句）。
> - 新增中正大學手語論文例句（`data/papers/paper_examples.jsonl`，可訓練 127 句）。
> - 合成句不再受 `teacher_train_eligible` 限制。
> - test 仍保留自有 33 句真實錄影句，維持與 Stage A/B 的可比性。
> - 切分結果：train 6,102／dev 641／test 33。
>
> **代價要知道**：語料庫全進訓練後，**不再有 584 句真實語料的泛化測試集**；
> 自動指標只剩 33 句核心 test（樣本小、BLEU 不穩）。要證明泛化能力，
> 需另外保留測試資料或改採人工評估。
>
> 以下 §3 舊內容保留作歷史紀錄（v4／v5 即以此切分訓練）。

...
- **人工評估已回收但結論受限**（2026-08-22，v14 盲測 100 題）。評分者
  99/100 偏好語料庫參考——模型的直譯腔在盲測下幾乎總是可辨識。但評分
  退化成二元偏好（98 題為全 1 對全 5，2／4 分未出現），且有 14 題僅差
  語序或一詞仍給最低分，口徑與量表定義不符，**5 分制多維度結果不可引用**；
  評分者身分與資歷亦未記載。詳見
  [results/human_eval_v14script_report.md](results/human_eval_v14script_report.md)。
  語意正確性在口徑釐清前，本卡仍不宣稱。

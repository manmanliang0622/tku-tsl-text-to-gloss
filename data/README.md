...
產生方式：`python3 scripts/synthesize.py`。

| 檔案 | 內容 |
|---|---|
| `synth/tsl_synth.jsonl` | 477 句合成平行資料，**全部 `review_status: pending`，未經審核不得進訓練集** |
| `synth/review_sheet.xlsx` | 給手語老師／聾人顧問的審核表（含審核結果、修正欄位） |

九個模板（T1–T9）各自標註 `rule_basis`（依據計畫 3.3 節的 7 條已查證語法規則＋自有實例句），槽位僅代入詞彙總表內的地名/時間詞，腳本會驗證不產生表外 Gloss、不與原始 35 句重複。`confidence` 兩級：

- `attested-pattern`（337 句）：句型直接複製自有標記表實例，只換地名槽位
- `rule-derived`（140 句，T7 身分是非問、T8 否定移動句、T9 WH 上班問句）：由規則組合推導、自有資料無實例，**審核優先**
...
3. 詞彙總表中 85 個 Gloss 是「自有影片資料集拍過的動作」上限；語言模型輸出若超出此表，下游動作庫檢索不到（計畫第 6.1 節「詞彙表內率」指標的依據）。

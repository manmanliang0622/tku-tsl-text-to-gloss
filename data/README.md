...
出處：文化部《臺灣手語語料庫（測試版）》。僅抓文字標記（Gloss／中文），不下載影片；**訓練與散布授權已確認合法（2026-08-04），須標明出處（© 文化部臺灣手語語料庫）**。

## refs/ — 語序查證證據

`refs/tslcorpus_evidence.jsonl`：文化部《臺灣手語語料庫》按主題關鍵詞抓取的例句（Gloss＋中文對照＋篇章代碼），© 文化部；訓練與散布授權已確認合法（2026-08-04），須標明出處。（全語料庫已由 `tslcorpus/parallel.jsonl` 完整涵蓋，此檔保留供 batch4 模板的規則依據回溯。）
...
出處：蔡素娟、戴浩一、劉世凱、陳怡君 2026《台灣手語線上辭典（中文版第五版）》，國立中正大學手語語言學台灣研究中心。僅抓後設資料與例句文字，不下載影片；**訓練與再散布授權已確認合法（2026-08-04），須註明上述出處**。詞條名的 `_A`/`_B` 後綴為方言／自由變異，`gloss` 欄已去除後綴（原名保留在 `name` 欄）。
...
4. **2026-08-05 同步核對**：本地 `twtsl` 已重新呼叫官網目前的公開 API。API 實際列出 3,500 個獨立詞條 ID、4,636 個中文名稱索引（去空白後 4,618 個不重複名稱），所以官網所稱「4,600 個詞彙」是名稱索引的約數，不是缺少 1,100 個詞條。例句 API 回傳 546 筆，移除 2 筆重複 `(Gloss, 中文翻譯)` 後為 544 句；官網「560 個常用詞之例句」不是去重句數口徑。現有快照已同步，無須等待額外 1,100 個詞條。

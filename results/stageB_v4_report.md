...
**解讀**：擴大 test 的可播放率距天花板僅 0.92 點——**任何模型端努力最多再拿 0.92 分**，因為天花板本身就是 70.89%。要提高這個數字只能**擴充下游可播放素材**（加入教育部辭典或其他已授權來源），屬**素材缺口**而非模型缺陷。2026-08-05 已重爬中正辭典並確認：官網約 4,600 詞是 4,636 個名稱索引的統計口徑，API 仍是 3,500 個獨立詞條 ID，因此單純重爬不會增加約 1,100 個詞條。合法詞率 85.14%（天花板 87.29%）則說明模型有 85% 的輸出是真實語料中出現過的合法用詞。

...
SUPERSEDED_BY = {
    ...
    # 2026-09-02 更新勝方：上游把 事情 換成 twtsl2_事（lexicon 記了
    # replaced_from=moc_G2D7P1.json）。重掃後 twtsl2_事 tier=ok（act_max 0.949、
    # 3.04 秒裡 2.89 秒有動作），moe_04_0090 由 severe 升到 poor（act_max 0.890、
    # 6.83 秒）但仍較差且過長。敗方不變，只換勝方。
    " 事情": ("事情", "moe_04_0090.json", "twtsl2_事.json"),
    ...
    # 傾聽：2026-09-02 從 BOTH_UNUSABLE 移過來。上游 twtsl:rescan-2026-09 換上
    # twtsl2_傾聽，重掃結果 tier=ok、act_max 1.0、4.03 秒全程有動作——不再是
    # 「兩支都不堪用」。moe_05_0022 仍是 severe（act_max 0.0，整段偵測不到動作）。
    "傾聽 ": ("傾聽", "moe_05_0022.json", "twtsl2_傾聽.json"),
}

# 同名兩支**都不堪用**者。不選任何一支（選了也演不出來），列在這裡是為了
# 讓它進補片待辦，而不是被當成「已處理」。
BOTH_UNUSABLE: dict[str, str] = {
    # 目前是空的。「傾聽」原本在這裡（moc_G2C27_R 與 moe_05_0022 兩支皆 severe），
    # 2026-09-02 上游換上 twtsl2_傾聽（重掃 tier=ok、4.03 秒全程有動作）後
    # 已移到 SUPERSEDED_BY。留著這張表是因為機制仍需要：同名兩支都不堪用時，
    # 要能讓它進補片待辦而不是被當成「已處理」。
}

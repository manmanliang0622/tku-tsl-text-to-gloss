...
只重算 NeedsReview / NeedsReview_calibrated——BLEU、ROUGE、違反率、
...
def load(path):
    """回傳 [(ref_needs_review, p_needs_review, greedy_needs_review)]。"""
    rows = []
    with open(path, encoding="utf-8") as fh:
        for line in fh:
            ...
            obj = json.loads(line)
            try:
                greedy = json.loads(obj["raw"]).get("needs_review")
            except Exception:
                greedy = None          # 破 JSON → 落回 greedy 時視為未預測
            rows.append((bool(obj.get("ref_needs_review")),
...
def main():
    ...
    for path in args.results:
        rows = load(path)
        print(f"=== {path}")
        print(json.dumps({"NeedsReview": greedy_block(rows),
                          "NeedsReview_calibrated": block(rows, args.threshold)},

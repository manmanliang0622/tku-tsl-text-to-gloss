...
TEMPLATES = [
    ...
    {"tid": "T31", "name": "讀書 WH 句〔問路〕",
     ...
     "gloss": ["你", "讀書", "哪裡"], "nms": NMS_WH, "slots": None,
     "confidence": "rule-derived",
     "rule_basis": "類比文化部語料庫 G2C31/a162 你哪裡畢業？→你/畢業/哪裡＋T9 句構；"
                   "詞形採語料庫用法「讀書」（語料 61 句，唸書僅 7 句；2026-08-04 校正）"},
    ...
    {"tid": "T33", "name": "市內搭乘句〔交通〕",
     ...
     "rule_basis": "文化部語料庫 G3C39/a143 你每天都坐這班公車上學嗎？→"
                   ...
                   "詞形採語料庫用法「公車」（語料 7 句，公共汽車 0 句；所引證據本身即用公車、讀書）"},
    ...
    {"tid": "T35", "name": "部位疼痛句〔看病〕",
     ...
     "gloss": ["我", "{P}", "痛"], "nms": None, "slots": BODY_PARTS,
     "confidence": "rule-derived",
     "rule_basis": "文化部語料庫 G1D13P1/a127 …我/右肩/抬起/痛/會（部位＋痛）；"
                   "詞形採語料庫用法「痛」（語料 17 句，疼 0 句；所引證據本身即用痛）"},
    {"tid": "T36", "name": "吃藥句〔看病〕",
     "chinese": ["我要吃藥。"],
...
def expand():
    ...
    for t in TEMPLATES:
        ...
        # 2026-08-04 校正：Gloss 改採**語料庫實際用法**而非辭典正式名。
        # 兩者多為同一辭典詞條的正式名／同義索引名，下游檢索皆可命中；
        # 但沿用語料用法可避免與 4,392 句語料庫訓練訊號矛盾。
